...
for prox in proxies:
    try:
        tt = requests.get('http://www.iac.es/galeria/westend/node15.html', proxies={"http":prox, "https":prox}, timeout=2)
        proxies_f.add(prox)
        print('add')
...
def get_comments_pag_ifexist(q, result, cycle_proxies):
    while not q.empty():
        ...
        try:
            ...
            a = requests.get('https://www.dailystrength.org'+url, proxies={"http":proxy, "https":proxy}, timeout=5)
...
cien_groups = [urls[(k)*3800:(k+1)*3800] for k in range(99)]
cien_groups.append(urls[99*3800:len(urls)])
...
def process_one(index,urls_in, num_threads):
    ...
    #save
    with open('post_threads_links_pagination'+str(index)+'.json', 'w') as out:
